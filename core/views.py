...
@login_required
def display_profile(h,user_x,u_key,username,profile_name):
    print("display User profile of"+str(username)+" with a key of "+str(profile_name)+"\n")
    the_x_user=User.objects.get(pk=u_key)
    the_x_profile=User_model.objects.get(user_of=u_key)
    return render(h,"user_profile/display_model.html",{'user':the_x_user,'user_profile':the_x_profile})
...
def schools_that_I_owne(h,user_key,user_name,profile_name,school_name):
    ...
    main_user=User_model.objects.get(pk=user_key)
    return render(h,"user_profile/model_5z.html",{'the_school':main_user})
...
def petrol_owned(h,user_x,user_key,user_name,profile_name):
    print("\npetrol station\n")
